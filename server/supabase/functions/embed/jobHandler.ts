...
export const documentJobHandler = async (job: Job) => {
    ...
    const { data, error } = await supa.from(sourceTable).select("*, files(*)").eq("id", entityId).single();
    ...
    // Get the file blob
    const fileBlob = await fileManagerInstance.buckets.usingBucket(bucket_name).getFileData(fileInBuckePath);

    if (!fileBlob) {
        throw new Error(`File not found in bucket - ${bucket_name} - ${fileInBuckePath}`);
    }
    ...
    const { error: embeddingError } = await supa.from("embeddings").insert(flattenedEmbeddingEntries);
...
async function blobToPageEmbeddings(blob: Blob): Promise<PageResult[]> {
    ...
    for (let i = 0; i < batches; i++) {
        ...
        console.log(`Processed batch ${i + 1} of ${batches} aka ${batch.length} documents`);
    }
...
const handlePage = async (document: Document): Promise<PageResult> => {
    ...
    for (let i = 0; i < parts; i++) {
        const part = contentWords.slice(i * ACTUAL_MAX_WORDS, (i + 1) * ACTUAL_MAX_WORDS).join(" ");
