...
export function useFetcher<T extends z.ZodType>({
  fetchParams,
  options,
}: {
  fetchParams: FetcherParams;
  options: FetcherOptions<T>;
}) {
  const [validatedData, setValidatedData] = useState<z.infer<T> | undefined>(
    undefined,
  );
  ...
  const fetcher = async (url: string, ...args: FetcherParams) => {
    if (!token) {
      throw new Error(
        "No token found - useFetcher must be used within a UserContextProvider",
      );
    }
  ...
  useEffect(() => {
    if (data) {
      ...
      if (parsedData.success) {
        ...
        console.log(
          "Investigate why expected data for",
          fetchParams[0],
          "is not valid",
        );
