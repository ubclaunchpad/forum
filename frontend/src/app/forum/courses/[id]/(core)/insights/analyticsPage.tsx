...
function PopularQuestionItem({ question, count, topics }: { 
  question: string; 
  count: number; 
  topics: string[];
}) {
  return (
    <div className="flex flex-col gap-1 py-2 border-b border-neutral-100 last:border-0">
      <div className="flex flex-row gap-2 items-center">
        ...
        <Badge variant="outline" className="ml-auto bg-primary-100 text-primary-600 border-primary-200">
          {count}×
        </Badge>
      </div>
      {topics.length > 0 && (
        <div className="flex flex-wrap gap-1 ml-7">
          {topics.map((topic, i) => (
            <Badge key={i} variant="outline" className="bg-neutral-100 text-neutral-600 px-2 py-0.5 border-neutral-200">
              {topic}
            </Badge>
          ))}
        </div>
...
function ActionItem({ title, description, severity, details }: {
  title: string;
  description: string;
    ...
    <div className="flex flex-col gap-2 py-3 border-b border-neutral-100 last:border-0">
      <div className="flex items-center gap-2">
        <AlertTriangle className={cn(
          ...
          severity === "high" ? "text-orange-500" : "text-red-500"
        )} />
...
function PopularSourceItem({ type, id, count }: {
  ...
  return (
    <div className="flex items-center gap-2 py-2 border-b border-neutral-100 last:border-0">
      ...
      <span className="text-neutral-800 truncate" title={id}>{id}</span>
      <Badge variant="outline" className="ml-auto bg-primary-100 text-primary-600 border-primary-200">
        {count}
      </Badge>
    </div>
...
export default function AnalyticsPage({analytics, loading}: {analytics: AnalyticsOutput | null, loading: boolean}) {
    const {token} = useContext(userContext);
    const {toast} = useToast();
    const course = useCourseStore(state => state.course);
    async function generateReport() {
        const res = await fetch(`${getApiUrl()}/search/courses/${course.id}/searches/report`, {
            method: "POST",
            headers: {
        ...
        if (res.ok) {
            toast({
                ...
                description: "The report has been generated and is available in the reports section",
            });
        } else {
            toast({
    ...
    if (!analytics) {
        return (
            <div className="flex justify-center items-center p-8">
                ...
                <h1 className="text-2xl font-medium text-neutral-600">No analytics data available</h1>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-6 w-full pb-10 flex-1 overflow-y-auto px-10 py-10">
            <div className="flex flex-col gap-2">
                ...
                <p className="text-neutral-600">Course insights and user engagement metrics</p>
                <Button onClick={generateReport}>Generate Report</Button>
                    ...
                    <div className="relative flex items-center justify-center">
                        ...
                        <div className="w-32 h-32 rounded-full bg-primary-50 border-2 border-primary-200 flex items-center justify-center shadow-md">
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-sm font-medium text-primary-700">Questions</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative flex items-center justify-center">
                        ...
                        <div className="w-32 h-32 rounded-full bg-primary-50 border-2 border-primary-200 flex items-center justify-center shadow-md">
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-sm font-medium text-primary-700">Users</span>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 text-neutral-600">
                    ...
                    <div className="relative flex items-center justify-center">
                        ...
                        <div className="w-32 h-32 rounded-full bg-primary-50 border-2 border-primary-200 flex items-center justify-center shadow-md">
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-sm font-medium text-primary-700">Sources</span>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 text-neutral-600">
                            ...
                            <ScrollArea className="h-80">
                                <div className="flex flex-col pr-1">
                                    {aiInsights.popularQuestions.length > 0 ? (
                                        ...
                                        <p className="text-neutral-600 text-sm py-2">No popular questions recorded yet</p>
                                    )}
                                </div>
                            </ScrollArea>
                        </CardContent>
                        ...
                        <CardContent>
                            <div className="flex flex-col gap-4">
                                {aiInsights.actionItems.length > 0 ? (
                                    <div>
                                        <h4 className="font-medium text-neutral-700 mb-2">For Instructors</h4>
                                        ...
                                        <h4 className="font-medium text-neutral-700 mb-2">For Students</h4>
                                        <ScrollArea className="h-60">
                                            <div className="flex flex-col pr-1">
                                                {aiInsights.actionItems.flatMap(item => 
                                    ...
                                    <p className="text-neutral-600 text-sm py-2">No action items available</p>
                                )}
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent>
                            <div className="flex flex-col gap-3">
                                <div className="flex items-center justify-between py-2 border-b border-neutral-100">
                                    ...
                                    <span className="font-medium text-neutral-800">{userEngagement.totalUsers}</span>
                                </div>
                                <div className="flex items-center justify-between py-2 border-b border-neutral-100">
                                    <span className="text-neutral-600">Avg. Threads per User</span>
                                    <span className="font-medium text-neutral-800">{userEngagement.userInsights.threads}</span>
                                </div>
                                <div className="flex items-center justify-between py-2">
                                    <span className="text-neutral-600">Avg. Questions per User</span>
                                    <span className="font-medium text-neutral-800">{userEngagement.userInsights.questions}</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent className="flex flex-col gap-4">
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {aiInsights.general.language.map((lang, idx) => (
                                        <Badge key={idx} variant="outline" className="text-sm bg-neutral-100 text-neutral-600 border-neutral-200">
                                            {lang}
                                        </Badge>
                                    ))}
                                </div>
                            ...
                            <div>
                                ...
                                <p className="text-sm text-neutral-600">{aiInsights.general.learning}</p>
                            </div>
                            ...
                            <ScrollArea className="h-60">
                                <div className="flex flex-col pr-1">
                                    {popularSources.length > 0 ? (
                                        ...
                                        <p className="text-neutral-600 text-sm py-2">No popular sources recorded yet</p>
                                    )}
                                </div>
                            </ScrollArea>
                        </CardContent>
