    ...
    for migration in migration_files:
        if migration not in applied_migrations or not applied_migrations[migration]:
            ...
            try:
                ...
                if (response.data):
                    if (migration in applied_migrations and not applied_migrations[migration]):
                        supabase.table("migrations_log").update({"status": True}).eq("migration_id",migration).execute()
                    elif (migration not in applied_migrations):
                        supabase.table("migrations_log").insert({"migration_id": migration, "status": True}).execute()
                    print(f'Successfully applied {migration}')

                else:
                    if (migration in applied_migrations and not applied_migrations[migration]):
                        supabase.table("migrations_log").update({"status": False}).eq("migration_id",migration).execute()
                    elif (migration not in applied_migrations):
                        supabase.table("migrations_log").insert({"migration_id": migration, "status": False}).execute()
