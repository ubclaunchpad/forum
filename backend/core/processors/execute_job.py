...
def validate_job_spec(job_id: UUID) -> tuple[bool, JobSpecification | None, Job | None]:
    ...
    with get_db() as db:
        ...
        job_spec = db.query(JobSpecification).filter(
            JobSpecification.id == job.specification_id
        ).first()
        
        if not job_spec:
            print(f"No job specification found for specification_id {job.specification_id}")
            ...
            try:
                ...
                with get_db() as db:
                    if job_spec.failure_strategy == "retry" and job.retry_count < 3:
                        ...
                        update_job_status(db, job_id, "not started", job.retry_count + 1)
                    else:
                        print("Max retries exceeded or retry not enabled. Marking as failed.")
                        update_job_status(db, job_id, "failed")
                return None
