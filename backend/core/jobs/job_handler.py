    ...
    def add_job(params: AddJobParams) -> UUID:
        ...
        with get_db() as db:
            ...
            job_spec = (
                ...
                .filter(JobSpecification.id == params.specification_id)
                .first()
            )

            if not job_spec:
                raise ValueError(
                    f"Job specification {params.specification_id} not found"
                )
            ...
            if params.recurring:
                if not params.recurring_interval:
                    raise ValueError(
                        "Recurring interval must be specified for recurring jobs"
                    )
