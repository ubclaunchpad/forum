...
class JobScheduler:
    ...
    def create_next_job(self, db, job: Job) -> None:
        ...
        if job.recurring_end_date and datetime.now() >= job.recurring_end_date:
            logger.info(
                f"Recurring job {job.id} has reached end date, not creating next instance"
            )
            return
    ...
    def check_and_execute_jobs(self):
        ...
        with get_db() as db:
            ...
            pending_jobs = db.query(Job).filter(Job.status == "not started").all()
            ...
            for job in pending_jobs:
                try:
                    # Execute job
                    logger.info(
                        f"Executing job {job.id} (specification_id: {job.specification_id})"
                    )
                    logger.info(
                        f"Job details: priority={job.priority}, recurring={job.recurring}, params={job.params}"
                    )
                    ...
                    if result is not None and job.recurring:
                        logger.info(
                            f"Job {job.id} completed successfully, creating next instance"
                        )
                    ...
                    logger.error(
                        f"Failed to execute job {job.id}: {str(e)}", exc_info=True
                    )
                    continue
