    ...
    with get_db() as db:
        try:
            ...
            exec = insert(document_tags).values(
                doc_id=document.id, tag_id=tag.id, created_by=UUID(author_id)
            )
            db.execute(exec)
            db.commit()
            ...
            print(str(e))
            raise HTTPException(
                status_code=500, detail=f"Error adding tag to document: {str(e)}"
            )
...
def remove_document_tag(document_id: str, tag_id: str) -> GeneralResponse:
    ...
    with get_db() as db:
        try:
            ...
            db.rollback()
            raise HTTPException(
                status_code=500, detail=f"Error removing tag from document: {str(e)}"
            )
