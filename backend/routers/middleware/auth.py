        ...
        for protected_segments in self.split_protected_paths:
            ...
            if len(protected_segments) != len(request_path_segments):
                if not (
                    ...
                    and len(protected_segments) <= len(request_path_segments) + 1
                ):
                    continue
            ...
            for i, protected_segment in enumerate(protected_segments):
                ...
                if (
                    i >= len(request_path_segments)
                    or protected_segment != request_path_segments[i]
                ):
                    is_match = False
                    break
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if (
            ...
            if not email or not password:
                raise RuntimeError(
                    "DEV_USER_EMAIL and DEV_USER_PASSWORD must be set in .env"
                )
