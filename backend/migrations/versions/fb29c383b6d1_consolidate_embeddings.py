...
def upgrade() -> None:
    ...
    sa.CheckConstraint("chunk_type IN ('text', 'image', 'code') OR chunk_type IS NULL", name='valid_chunk_type'),
    sa.CheckConstraint("jsonb_typeof(chunk_metadata) = 'object' OR chunk_metadata IS NULL", name='valid_metadata'),
    sa.ForeignKeyConstraint(['parent_chunk_id'], ['public.embeddings.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    schema='public'
    )
    op.create_index('embeddings_vector_idx', 'embeddings', ['embedding'], unique=False, schema='public', postgresql_using='ivfflat', postgresql_with={'lists': 100}, postgresql_ops={'embedding': 'vector_cosine_ops'})
...
def downgrade() -> None:
    ...
    op.drop_constraint(None, 'course_documents', schema='public', type_='foreignkey')
    op.drop_index('embeddings_vector_idx', table_name='embeddings', schema='public', postgresql_using='ivfflat', postgresql_with={'lists': 100}, postgresql_ops={'embedding': 'vector_cosine_ops'})
