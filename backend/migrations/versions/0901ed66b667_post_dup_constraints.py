...
def upgrade() -> None:
    ...
    for i in range(1, 16):
        op.execute(f'ALTER TABLE public.posts DROP CONSTRAINT IF EXISTS posts_course_id_fkey{i}')
    
    # Drop all numbered duplicate constraints for created_by
    for i in range(1, 16):
        op.execute(f'ALTER TABLE public.posts DROP CONSTRAINT IF EXISTS posts_created_by_fkey{i}')
