...
def upgrade() -> None:
    ...
    op.add_column(
        "profiles", sa.Column("pronouns", sa.Text(), nullable=True), schema="public"
    )
    op.add_column(
        "profiles", sa.Column("username", sa.Text(), nullable=True), schema="public"
    )
    op.add_column(
        "profiles", sa.Column("bio", sa.Text(), nullable=True), schema="public"
    )
    op.add_column(
        "profiles",
        sa.Column("socials", JSONB(), server_default="{}", nullable=True),
        schema="public",
    )
    op.add_column(
        "profiles", sa.Column("timezone", sa.Text(), nullable=True), schema="public"
    )
    op.add_column(
        "profiles", sa.Column("display_name", sa.Text(), nullable=True), schema="public"
    )

    # Add unique constraint to username
    op.create_unique_constraint(
        "uq_profiles_username", "profiles", ["username"], schema="public"
    )
